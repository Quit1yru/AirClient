...
import net.ccbluex.liquidbounce.ui.font.AWTFontRenderer.Companion.MAX_CACHED_STRINGS
    ...
    fun drawString(text: String, x: Double, y: Double, color: Int) {
        ...
        for (char in text) {
            ...
            if (loc == null) {
                ...
                // Render with original color and WITHOUT shadow/background
                val currentFallbackW = mc.fontRendererObj.drawString(char.toString(), (currX / rev) + fallbackWidth, 1.0f, Color(color, true).rgb, false).toFloat()
                fallbackWidth += currentFallbackW
