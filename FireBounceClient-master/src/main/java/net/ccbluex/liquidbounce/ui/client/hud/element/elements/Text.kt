...
import net.ccbluex.liquidbounce.FireBounce.CLIENT_AUTHOR
import net.ccbluex.liquidbounce.FireBounce.CLIENT_NAME
import net.ccbluex.liquidbounce.FireBounce.clientCommit
...
open class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F, side: Side = Side.default()) : Element(
    "Text",
    x,
    y,
    scale,
    ...
    private val shadowK by boolean("GlowShadow", false)
    private val shadowStrength by float("Shadow-Strength", 1F, 0.5F..3F) { shadowK }
    private val shadowColor by color("Shadow-Color", Color(0, 0, 0, 120)) { shadowK }
    ...
    override fun drawElement(): Border {
        ...
        if (shadowK) {
            ...
            val bgX = rectPos[0]
            val bgY = rectPos[1]
            val bgWidth = scaledWidth - (if (shouldRender) 16F else 0F) + horizontalPadding * bgScale
            val bgHeight = scaledHeight + verticalPadding * bgScale

            // 绘制阴影
            if (!onScaffold || (onScaffold && Scaffold.handleEvents())) {
                drawGlow(
                    bgX, bgY, bgWidth, bgHeight,
                    (shadowStrength * 10).toInt(), shadowColor)
            }
        }

        assumeNonVolatile {
            if (((Scaffold.handleEvents() && onScaffold) || Scaffold2.handleEvents()) || !onScaffold || mc.currentScreen is GuiHudDesigner) {
