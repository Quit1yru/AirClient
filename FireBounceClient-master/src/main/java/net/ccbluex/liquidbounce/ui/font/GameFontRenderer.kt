...
import net.ccbluex.liquidbounce.utils.extra.ColorUtils.hexColors
import net.ccbluex.liquidbounce.utils.extra.ColorUtils.randomMagicText
...
class GameFontRenderer(
    ...
    val stringWidth: (String) -> Int
        get() = { text -> getStringWidth(text) }

    fun stringWidth(text: String): Int = getStringWidth(text)

    fun getMiddleOfBox(boxHeight: Float): Float = boxHeight / 2f - height / 2f

    fun drawCenteredStringShadow(text: String, x: Float, y: Float, color: Int) {
        val drawX = x - getStringWidth(text) / 2f
        drawStringWithShadow(text, drawX, y, color)
    }
    ...
    override fun drawString(
        ...
        if (shadow) {
            ...
            // Ensure blend is enabled for shadow
            glEnable(GL_BLEND)
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
            drawText(
                currentText,
                x + 1f,
                baseY + 1f,
                Color(0, 0, 0, minOf(60, color shr 24 and 0xFF)).rgb, // Lower shadow alpha to 60
                ignoreColor = true
            )
            glDisable(GL_BLEND) // Disable blend for main text if it's not transparent
        }
        // ...
