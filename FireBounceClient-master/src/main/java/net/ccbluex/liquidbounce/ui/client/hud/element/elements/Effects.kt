...
class Effects(
    ...
    private fun drawModernMode(): Border {
        ...
        val headerHeight = Fonts.fontExtraBold40.height.toFloat() + padding * 2
        var maxWidth = Fonts.fontExtraBold40.getStringWidth(headerText) + padding * 2
        ...
        for (effect in potions) {
            ...
            val nameWidth = Fonts.fontExtraBold40.getStringWidth(nameText)
            val durationWidth = Fonts.fontExtraBold40.getStringWidth(durationText)
            val lineWidth = nameWidth + durationWidth + padding * 3
            if (lineWidth > maxWidth) maxWidth = lineWidth
            localHeight += Fonts.fontExtraBold40.height.toFloat() + padding
        }
        ...
        when (titleAlign) {
            "Center" -> Fonts.fontExtraBold40.drawCenteredString(headerText, posX + maxWidth / 2, titleY, Color.WHITE.rgb)
            "Left"   -> Fonts.fontExtraBold40.drawString(headerText, posX + padding, titleY, Color.WHITE.rgb)
            "Right"  -> Fonts.fontExtraBold40.drawString(headerText, posX + maxWidth - Fonts.fontExtraBold40.getStringWidth(headerText) - padding, titleY, Color.WHITE.rgb)
        }

        val imagePosX = if (titleAlign == "Right") posX + padding else posX + maxWidth - iconSizeX - padding
        Fonts.fontExtraBold40.drawString("E", imagePosX + 2f, headerY + 7f + 2, getTextColor(0))

        posY += Fonts.fontExtraBold40.height.toFloat() + padding * 2
        ...
        for (effect in potions) {
            ...
            val durationText = Potion.getDurationString(effect)
            val durationWidth = Fonts.fontExtraBold40.getStringWidth(durationText)
            Fonts.fontExtraBold40.drawString(nameText, posX + padding, posY + 2, Color.WHITE.rgb)
            Fonts.fontExtraBold40.drawString(durationText, posX + maxWidth - padding - durationWidth, posY + 2, Color.WHITE.rgb)
            posY += Fonts.fontExtraBold40.height.toFloat() + padding
        }
    ...
    private fun drawFDPMode(): Border {
        ...
        for (potionEffect in activePotions) {
            ...
            Fonts.Bold36.drawString(Potion.getDurationString(potionEffect), 29F, pY + 4.0F, displayTextColor)
