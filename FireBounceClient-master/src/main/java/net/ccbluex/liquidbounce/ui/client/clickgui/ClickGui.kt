...
import net.ccbluex.liquidbounce.ui.client.clickgui.style.styles.NewStyle
...
object ClickGui : GuiScreen() {
    ...
    private fun setupTargetsPanel(xPos: Int = 100, yPos: Int, width: Int, height: Int) =
        Panel("Targets", xPos, yPos, width, height, false, listOf(
            ...
            ButtonElement("BedWarsMob", { if (Targets.bedwarsMob) guiColor else Int.MAX_VALUE }) {
                Targets.bedwarsMob = !Targets.bedwarsMob
            },
    ...
    override fun drawScreen(x: Int, y: Int, partialTicks: Float) {
        ...
        assumeNonVolatile {
            ...
            if (style is NewStyle) {

                style.drawPanel(mouseX, mouseY, panels.firstOrNull() ?: return@assumeNonVolatile)
            } else {
                for (panel in panels) {
            ...
            if (Mouse.hasWheel()) {
                ...
                if (wheel != 0) {
                    ...
                    if (style is NewStyle) {
                        (style as NewStyle).handleScroll(wheel)
                        handledScroll = true
                    } else {
    ...
    public override fun mouseClicked(x: Int, y: Int, mouseButton: Int) {
        ...
        if (style is NewStyle) {
            if ((style as NewStyle).handleClick(mouseX, mouseY, mouseButton)) {
                return
            }
        } else {
            // Handle foremost panel.
    ...
    public override fun mouseReleased(x: Int, y: Int, button: Int) {
        ...
        if (style is NewStyle) {
            (style as NewStyle).handlePanelRelease()
        } else {
    ...
    override fun keyTyped(typedChar: Char, keyCode: Int) {
        ...
        if (style is NewStyle) {
            if ((style as NewStyle).handleKeyTyped(keyCode)) {
                return
            }
        }
