...
import net.ccbluex.liquidbounce.utils.extra.ColorUtils.blendColors
import net.ccbluex.liquidbounce.utils.extra.ColorUtils.minecraftRed
...
class EditorPanel(private val hudDesigner: GuiHudDesigner, var x: Int, var y: Int) : MinecraftInstance {
    ...
    private fun drawCreate(mouseX: Int, mouseY: Int) {
        ...
        for ((element, info) in ELEMENTS) {
            ...
            Bold36.drawString(name, x + 2f, y + height.toFloat(), Color.WHITE.rgb)

            val stringWidth = Bold36.getStringWidth(name) + 8
        ...
        Bold36.drawCenteredString("§lCreate element", centerX, y + 3.5F, Color.WHITE.rgb)
    }
    ...
    private fun drawSelection(mouseX: Int, mouseY: Int) {
        ...
        Bold36.drawString("§lCreate element", x + 2f, y.toFloat() + height, Color.WHITE.rgb)
        ...
        Bold36.drawString("§lReset", x + 2f, y.toFloat() + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX in x..x + width && mouseY in y + height..y + height + 10) {
        ...
        Bold36.drawString("§lAvailable Elements", x + 2f, y + height.toFloat(), Color.WHITE.rgb)
        height += 10
        realHeight += 10

        for (element in HUD.elements) {
            Bold36.drawString(element.name, x + 2, y + height, Color.WHITE.rgb)

            val stringWidth = Bold36.getStringWidth(element.name) + 8
        ...
        val centerX = (x..x + width).lerpWith(0.5F)
        Bold36.drawCenteredString("§lElement Editor", centerX, y + 3.5f, Color.WHITE.rgb)

        if (showConfirmation) {
            val confirmationMessage = "Are you sure you want to reset?"
            val textWidth = Bold36.getStringWidth(confirmationMessage)
            ...
            Bold36.drawCenteredString(confirmationMessage, centerDialogX, dialogY + 12f, Color.WHITE.rgb)
            ...
            val answerButtonY = (dialogY + 12 + Bold36.height..dialogY + dialogHeight + 10).lerpWith(0.5F)
            val buttonWidth = Bold36.getStringWidth("Yes")
            ...
            buttonData.forEach { (labelAndColor, bounds) ->
                ...
                Bold36.drawCenteredString(label, buttonX, answerButtonY - 2, Color.WHITE.rgb, true)
    ...
    private fun drawEditor(mouseX: Int, mouseY: Int) {
        ...
        // Scale
        Bold36.drawString("Scale: ${"%.2f".format(element.scale)}", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Horizontal
        Bold36.drawString("H:", x + 2, y + height, Color.WHITE.rgb)
        Bold36.drawString(
        ...
        // Vertical
        Bold36.drawString("V:", x + 2, y + height, Color.WHITE.rgb)
        Bold36.drawString(
        ...
        for (value in element.values) {
            ...
            when (value) {
                is BoolValue -> {
                    ...
                    val stringWidth = Bold36.getStringWidth(value.name)
                ...
                is FloatValue -> {
                    ...
                    Bold36.drawString(text, x + 2, y + height, Color.WHITE.rgb)

                    val stringWidth = Bold36.getStringWidth(text)
                    if (width < stringWidth + 8) width = stringWidth + 8
                ...
                is ListValue -> {
                    // Title
                    Bold36.drawString(value.name, x + 2, y + height, Color.WHITE.rgb)
                ...
                is FontValue -> {
                    ...
                    Bold36.drawString(displayString, x + 2, y + height, Color.WHITE.rgb)

                    val stringWidth = Bold36.getStringWidth(displayString)
                    if (width < stringWidth + 8) width = stringWidth + 8
                ...
                is ColorValue -> {
                    ...
                    val optimalWidth = (Bold36.getStringWidth(combinedText) * 1.5F).roundToInt()
                    ...
                    val rgbaOptionHeight = if (value.showOptions) Bold36.height * 4 else 0
                    ...
                    val valueX = startX + Bold36.getStringWidth(startText)
                    val valueWidth = Bold36.getStringWidth(valueText)

                    if (rightClickPressed && mouseX in valueX..valueX + valueWidth && mouseY.toFloat() in textY - 2..textY + Bold36.height - 3F) {
                    ...
                    val widestLabel = rgbaLabels.maxOf { Bold36.getStringWidth(it) }
                    ...
                    hudDesigner.elementEditableText?.chosenText?.let {
                        ...
                        val cursorY = textY + value.rgbaIndex * Bold36.height + 10

                        if (it.selectionActive()) {
                            val start = startValueX + Bold36.getStringWidth(it.string.take(it.selectionStart!!))
                            val end = startValueX + Bold36.getStringWidth(it.string.take(it.selectionEnd!!))
                            drawRect(
                                start,
                                cursorY - 3f,
                                end,
                                cursorY + Bold36.fontHeight - 2,
                                Color(7, 152, 252).rgb
                            )
                        }

                        highlightCursor = {
                            val cursorX = startValueX + Bold36.getStringWidth(it.cursorString)
                            drawRect(
                                cursorX,
                                cursorY - 3F,
                                cursorX + 1F,
                    ...
                    if (value.showOptions) {
                        ...
                        val maxWidth = Bold36.getStringWidth("255")
                        ...
                        rgbaLabels.forEachIndexed { index, label ->
                            ...
                            val yPosition = rgbaYStart + index * Bold36.height
                            ...
                            Bold36.drawString(label, textX, yPosition, Color.WHITE.rgb)
                            Bold36.drawString(defaultText, finalX, yPosition, defaultColor.rgb)
        ...
        // Header
        drawRect(x, y, x + width, y + 12, guiColor)
        Bold36.drawString("§l${element.name}", x + 2F, y + 3.5F, Color.WHITE.rgb)

        // Delete button
        if (!element.info.force) {
            val deleteWidth = x + width - Bold36.getStringWidth("§lDelete") - 2
            Bold36.drawString("§lDelete", deleteWidth.toFloat(), y + 3.5F, Color.WHITE.rgb)
