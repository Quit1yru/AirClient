...
class SideGui : GuiPanel() {
    ...
    private val categories = arrayOf("UI", "Configs")
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float, alpha: Int) {
        ...
        // Close button
        val closeX = drag!!.x + rectWidth - 30
        val closeY = drag!!.y + 15
        val closeHovered = DrRenderUtils.isHovering(closeX - 5, closeY - 5, 25f, 25f, mouseX, mouseY)
        Fonts.InterBold_26.drawString("X", closeX, closeY, if (closeHovered) Color(255, 100, 100, alpha).rgb else Color(0, 0, 0, (alpha * 0.8f).toInt()).rgb)
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, button: Int) {
        ...
        val closeX = drag!!.x + rectWidth - 30
        val closeY = drag!!.y + 15
        if (DrRenderUtils.isHovering(closeX - 5, closeY - 5, 25f, 25f, mouseX, mouseY) && button == 0) {
            focused = false
            return
        }
        ...
        if (currentCategory == "UI") {
             ...
             val buttonX = drag!!.x + rectWidth / 2 - buttonWidth / 2
             val buttonY = drag!!.y + rectHeight / 2 - buttonHeight / 2
             if (DrRenderUtils.isHovering(buttonX, buttonY, buttonWidth, buttonHeight, mouseX, mouseY) && button == 0) {
                 FDPState.resetPositions()
             }
        }
    ...
    private fun drawUiCategory(mouseX: Int, mouseY: Int, alpha: Int) {
        ...
        val isHovered = DrRenderUtils.isHovering(buttonX, buttonY, buttonWidth, buttonHeight, mouseX, mouseY)
        val buttonColor = if (isHovered) Color(70, 70, 70, alpha).rgb else Color(50, 50, 50, alpha).rgb

        DrRenderUtils.drawRect2(buttonX.toDouble(), buttonY.toDouble(), buttonWidth.toDouble(), buttonHeight.toDouble(), buttonColor)
        Fonts.InterBold_26.drawCenteredString("Reset Position", buttonX + buttonWidth / 2, buttonY + buttonHeight / 2 - Fonts.InterBold_26.height / 2, DrRenderUtils.applyOpacity(-1, alpha / 255f))
    }
    ...
    private fun drawMainPanel(sr: ScaledResolution, alpha: Int, mouseX: Int, mouseY: Int): Color {
        ...
        if (showSideOutline) {
            val outlineColor = ClickGUI.generateColor(0) // 使用ClickGUI中的generateColor方法
    ...
    private fun drawCategoryTabs(mouseX: Int, mouseY: Int, alpha: Int) {
        ...
        categories.forEachIndexed { index, cat ->
            ...
            val colorToInterpolate = DrRenderUtils.applyOpacity(ClickGUI.generateColor(index).rgb, alpha / 255f) // 使用ClickGUI中的generateColor方法
    ...
    private fun drawOverlays(sr: ScaledResolution, alpha: Int, mouseX: Int, mouseY: Int) {
        ...
        DrRenderUtils.drawGradientRectSideways2(
            ...
            DrRenderUtils.applyOpacity(ClickGUI.generateColor(colorIndex).rgb, 0f), // 使用ClickGUI中的generateColor方法
            DrRenderUtils.applyOpacity(ClickGUI.generateColor(colorIndex).rgb, 0.4f * moveAnimOut)
        )
