...
class HotKeys(
    ...
    override fun drawElement(): Border {
        AWTFontRenderer.assumeNonVolatile {
            ...
            Fonts.fontExtraBold40.drawCenteredStringWithShadow(titleText, posX + (currentWidth / 2f), posY + padding, Color.WHITE.rgb)
            if (iconEnabled) {
                val iconSize = 10f
                val iconDrawColor = if (iconColorMode == "Custom") iconColor.rgb else getTextColor()
                Fonts.fontExtraBold40.drawString(iconText, posX + currentWidth - iconSize - padding, posY + padding + 2f, iconDrawColor)
            }
            ...
            for (module in moduleManager) {
                ...
                val bindWidth = Fonts.fontExtraBold40.getStringWidth(bindText)
                ...
                Fonts.fontExtraBold40.drawString(nameText, posX + padding, posY + 2f, moduleDrawColor)
                Fonts.fontExtraBold40.drawString(bindText, posX + currentWidth - bindWidth - padding, posY + 2f, keysDrawColor)
                val lineHeight = Fonts.fontExtraBold40.height + padding
                posY += lineHeight
