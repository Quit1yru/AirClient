...
//嘲讽文案有些是AI写的，有些是照搬的其他端的
object AutoL : Module("AutoL", Category.FUN) {
    /**
     * @auther polybeta
     * @description 自动嘲讽
     */
    private val wordPattern by choices(
        "WordPattern",
        arrayOf("Poem", "Mama", "PridePlus", "Cixiaogui", "CrystalPVP","Clear","English","SanGuo","Custom"),
        "Poem"
    )
    private val shout by boolean("Shout",false)
    private val shoutMode by choices("ShoutMode",arrayOf("Command","ExclamationMark","""@"""),"ExclamationMark")
    private val customRawText by text("CustomText","") {wordPattern == "Custom"}
    ...
    fun onAttackEvent(event: AttackEntityEvent) {
        ...
        if (target is EntityPlayer && !target.isDead && !enemies.contains(target)) {
            enemies.add(target)
        }
    }
    ...
    private val cixiaogui = listOf(
        ...
        "呐呐~杂鱼先生今天的败北姿势比昨天有创意呢~要给你颁发进步奖吗？♡",
        "哎呀呀~这么快就投降啦？咱特制的败北台词才念到第三页呢~",
        "你挣扎的样子真像落在窗台上的小雨点~滴滴答答的...最后还是要消失呢♡",
        "知道吗？连隔壁家三花猫都在学你气鼓鼓的表情~不过它演得更可爱些哟~",
        "这把游戏咱用左手玩的哦~什么？你问右手在做什么？当然在给你写挽联啦~",
        "失败记录又更新了呢！要帮你申请'最持之以恒输家'的吉尼斯纪录吗？♡",
        "刚才那波操作让咱想起被风吹跑的塑料袋~飘来飘去的完全找不到方向呢~",
        "你认输的声音比蚊子哼哼还细微~要不要借你个喇叭呀？当然要收利息的~♡",
        "看呐看呐~连游戏角色都在说\"请放过这个可怜人吧\"...真是的~咱还没玩尽兴呢~",
        "你的战术词典里是不是只有\"向前冲\"三个字呀？连蒲公英都比你懂得迂回呢~",
        "咱特意准备了999种赢你的方法~真遗憾你连第1种都撑不过去呢♡",
    )
    ...
    private fun sayL(entity: Entity) {
        val shoutMark = if (shout && shoutMode == """@""") "@" else if (shout && shoutMode == "Command") "/shout " else if (shout && shoutMode == "ExclamationMark") "!" else ""
        ...
        val messageWithName = if (nameInFront) "$shoutMark$playerName $rawMessage" else "$shoutMark$rawMessage"
