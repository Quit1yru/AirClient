...
object AntiBot : Module("AntiBot", Category.MISC) {
    ...
    private val matrixTest by boolean("MatrixTest",false) {matrix}
    ...
    fun isBot(entity: EntityLivingBase): Boolean {
        ...
        if (matrix) {
            if (entity.displayName.formattedText.replace("§r", "").isEmpty()) {
                return true
            }
            if (matrixTest) {
                if (entity.onGround && entity.fallDistance > 3.0) {
                    return true
                }
                if (!entity.onGround && entity.ticksExisted > 20 && entity.motionY == 0.0) {
                    return true
                }
            }
        }
