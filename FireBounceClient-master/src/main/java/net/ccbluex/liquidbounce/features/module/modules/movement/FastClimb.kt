...
object FastClimb : Module("FastClimb", Category.MOVEMENT) {
    ...
    val mode by choices(
        "Mode",
        arrayOf("Vanilla", "Delay", "Clip", "AAC3.0.0", "AAC3.0.5", "SAAC3.1.2", "AAC3.1.2", "GrizzlyLatest"), "Vanilla"
    )
    ...
    val onMotion = handler<MotionEvent> { event ->
        ...
        when{
            mode == "GrizzlyLatest" && thePlayer.isCollidedHorizontally && thePlayer.isOnLadder -> {
                mc.thePlayer.motionY = 0.19
                if (mc.thePlayer.ticksExisted % 2 == 1) {
                    val yaw: Double = MovementUtils.direction
                    MovementUtils.resetMotion(false)
                    event.x=(event.x - -MathHelper.sin(yaw.toFloat()) * 0.15f)
                    event.z=(event.z - MathHelper.cos(yaw.toFloat()) * 0.15f)
                }
            }
        }
