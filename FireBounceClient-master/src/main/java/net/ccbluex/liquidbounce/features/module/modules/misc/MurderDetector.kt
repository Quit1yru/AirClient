...
object MurderDetector : Module("MurderDetector", Category.MISC, gameDetecting = false) {

    private val showText by boolean("ShowText", true)
    private val chatValue by boolean("Chat", true)
    private val sendMessage by boolean("Send chat message", true)
                    ...
                    if (murder1 == null) {
                        if(sendMessage)
                            mc.thePlayer.sendChatMessage(
                                (EnumChatFormatting.YELLOW.toString() + player.name + EnumChatFormatting.WHITE.toString() + " is " + EnumChatFormatting.RED.toString() + "Murderer"
                                        + EnumChatFormatting.WHITE.toString() + "! Position: (" + EnumChatFormatting.GREEN.toString() + player.posX.toInt() + ", " + player.posY.toInt() + ", " + player.posZ.toInt() + ")" +
                                        " " + EnumChatFormatting.WHITE.toString() + " | From "+ EnumChatFormatting.RED.toString() + "[FireBounce]").replace("§", "&")
                            )
                        if (chatValue)
