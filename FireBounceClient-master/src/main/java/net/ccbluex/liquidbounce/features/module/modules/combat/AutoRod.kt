...
object AutoRod : Module("AutoRod", Category.COMBAT) {
    ...
    private val activationDistance by floatRange("ActivationDistance", 3f..8f, 1f..8f)
    ...
    private val pushDelay by intRange("PushDelay", 100..100, 50..1000,"ms")
    private val pullbackDelay by intRange("PullbackDelay", 500..500, 50..1000,"ms")

    private val onUsingItem by boolean("OnUsingItem", false)
    private val onlyOnKillAura by boolean("OnlyOnKillAura",false)
    private val DisSetInventory by boolean("SetInventorySlotOnDisable",false)
    private val DisSetInventorySlot by int("SetInventorySlotOnDisable-Slot",0,0..8) {DisSetInventory}
    private val RangeDebugger by boolean("RangeDebugger",false)
    private val switchBackAfterUse by boolean("SwitchBackAfterUse", true)
    
    private val pullWhenTargetHurt by boolean("PullWhenTargetHurt", false)
    private val targetHurtTime by int("TargetHurtTime", 9, 0..10) { pullWhenTargetHurt }
    ...
    override val tag: String?
        get() = "${activationDistance.start} - ${activationDistance.endInclusive}"
    ...
    val onUpdate = handler<UpdateEvent> {
        ...
        if (usingRod && pullWhenTargetHurt && target != null && target!!.hurtTime >= targetHurtTime) {
            if (switchBack != -1 && mc.thePlayer.inventory.currentItem != switchBack && switchBackAfterUse) {
        ...
        if (usingRod) {
            ...
            if (rodPullTimer.hasTimePassed(pullbackDelay.random())) {
                if (switchBack != -1 && mc.thePlayer.inventory.currentItem != switchBack && switchBackAfterUse) {  // 添加条件判断
                    // Switch back to previous item
                    mc.thePlayer.inventory.currentItem = switchBack
                    mc.playerController.syncCurrentPlayItem()
                } else {
            ...
            if (facingEnemy && getHealth(mc.thePlayer, healthFromScoreboard, absorption) >= playerHealthThreshold) {
                ...
                if (facingEntity == null) {
                    ...
                    facingEntity = RaycastUtils.raycastEntity(activationDistance.endInclusive.toDouble()) { isSelected(it, true) }
                }
                ...
                if (isSelected(facingEntity, true)) {
                    val distance = facingEntity?.let { entity -> mc.thePlayer.getDistanceToEntityBox(entity) }?: Double.MAX_VALUE
                    val realdistance = BigDecimal(distance.toString()).setScale(2, RoundingMode.HALF_DOWN).toFloat()
                    if (RangeDebugger) {
                        chat("$realdistance | ${activationDistance.start.toDouble()}")
                    }
                    if (distance >= activationDistance.start.toDouble()) {
                        ...
                        if (nearbyEnemies.size <= enemiesNearby) {
                            ...
                            if (ignoreOnEnemyLowHealth) {
                                ...
                                rod = true
                                // 设置当前目标
                                target = facingEntity as? EntityLivingBase
                            }
                        }
                    }
                }
            ...
            if (rod && pushTimer.hasTimePassed(pushDelay.random())) {
    ...
    private fun getAllNearbyEnemies(): List<Entity> {
        ...
        return mc.theWorld.loadedEntityList.filter {
            isSelected(it, true) && player.getDistanceToEntityBox(it) < activationDistance.endInclusive.toDouble() && player.getDistanceToEntityBox(it) > activationDistance.start.toDouble()
        }
    }
    ...
    override fun onDisable() {
        target = null
        if (DisSetInventory) {
            mc.thePlayer.inventory.currentItem = DisSetInventorySlot
            mc.playerController.syncCurrentPlayItem()
        }
        super.onDisable()
    }
