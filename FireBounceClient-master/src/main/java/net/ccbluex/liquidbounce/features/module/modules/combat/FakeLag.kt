...
object FakeLag : Module("FakeLag", Category.COMBAT, gameDetecting = false) {
    ...
    private val tagMode by choices("TagMode",arrayOf("PacketCount","Custom","None"),"PacketCount")
    private val customTagText by text("TagText","") {tagMode == "Custom"}


    private val delay by int("Delay", 550, 0..5000,"ms")
    private val recoilTime by int("RecoilTime", 750, 0..5000,"ms")

    private val allowedDistToEnemy by floatRange("MinAllowedDistToEnemy", 1.5f..3.5f, 0f..6f)

    private val blinkOnAttack by boolean("BlinkOnAttack", true)
    private val blinkOnInteract by boolean("BlinkOnInteract",true)
    private val blinkOnHurt by boolean("BlinkOnHurt",true)
    private val blinkOnKnockBack by boolean("BlinkOnKnockBack",true)
    private val blinkOnUsingItem by boolean("BlinkOnUsingItem",false)
    private val blinkOnEating by boolean("BlinkOnEating",false) {!blinkOnUsingItem}
    private val blinkOnScaffolding by boolean("BlinkOnScaffolding",false)
    ...
    private val onlyWorkWhenInRange by boolean("OnlyWorkWhenInRange",false) {onlyKillAura}
    private val rangeLimit by floatRange("RangeLimit",0f..4.5f,0f..10.0f) {onlyKillAura && onlyWorkWhenInRange}
    ...
    private val pauseOnS08 by boolean("PauseOnS08",true)
    private val pauseTick by int("PauseTicks",10,0..20,"Ticks") {pauseOnS08}
    ...
    val scaffolding: Boolean
        get() = (Scaffold.handleEvents() && Scaffold.placeRotation != null) || (Scaffold2.handleEvents() && Scaffold2.placeRotation != null)
    private var renderData = ModelRenderData1(Vec3_ZERO, Rotation.ZERO)
    ...
    val onPacket = handler<PacketEvent> { event ->
        ...
        if (mc.thePlayer.ticksExisted < 10 || pauseTicks != 0) {
            blink()
            return@handler
        }
        ...
        if (mc.thePlayer.isUsingItem) {
            ...
            if (blinkOnEating && mc.thePlayer.heldItem?.item is ItemFood) {
                blink()
                return@handler
            }
        }
        ...
        if (onlyKillAura) {
            if (!KillAura.handleEvents()) {
                ...
                blink()
                return@handler
            } else if (mc.thePlayer.getDistanceToEntityBox(KillAura.target?: return@handler) !in rangeLimit && onlyWorkWhenInRange) {
                blink()
                return@handler
            }
        }
        ...
        if (player.health < player.maxHealth && blinkOnHurt) {
        ...
        if (scaffolding && blinkOnScaffolding) {
            blink()
            return@handler
        }
        ...
        if (packet is C02PacketUseEntity) {
            when (packet.action) {
                C02PacketUseEntity.Action.INTERACT -> if (blinkOnInteract) blink()
                C02PacketUseEntity.Action.ATTACK -> if (blinkOnAttack) blink()
                C02PacketUseEntity.Action.INTERACT_AT -> if (blinkOnInteract) blink()
            }
            return@handler
        }
        ...
        when (packet) {
            ...
            is S08PacketPlayerPosLook, is C08PacketPlayerBlockPlacement, is C07PacketPlayerDigging, is C12PacketUpdateSign, is C19PacketResourcePackStatus -> {
                if (packet is S08PacketPlayerPosLook) pauseTicks = pauseTick
                blink()
                return@handler
            }
    ...
    val onGameLoop = handler<GameLoopEvent> {
        ...
        // 如果启用了onlyKillAura且KillAura未运行，则不执行FakeLag
        if (onlyKillAura && !KillAura.handleEvents()) {
            blink()
            return@handler
        }
...
data class ModelRenderData1(var pos: Vec3, var rotation: Rotation) {
