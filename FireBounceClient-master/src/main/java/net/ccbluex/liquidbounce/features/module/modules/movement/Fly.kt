...
import net.ccbluex.liquidbounce.features.module.modules.movement.flymodes.aac.AAC5
...
import net.ccbluex.liquidbounce.features.module.modules.movement.flymodes.matrix.*
...
import net.ccbluex.liquidbounce.features.module.modules.movement.flymodes.vanilla.Fastest
...
import net.ccbluex.liquidbounce.features.module.modules.movement.flymodes.vulcan.VulcanBoost
...
object Fly : Module("Fly", Category.MOVEMENT, Keyboard.KEY_F) {
    private val flyModes = arrayOf(
        ...
        // Vulcan
        Vulcan, VulcanOld, VulcanGhost, VulcanBoost,
        ...
        // Matrix
        Matrix, MatrixLag, MatrixBoost, MatrixTest, MatrixTest2,

        // Tatako
        TatakoFly, Tatako0972Fly,

        // Other
        Jetpack, KeepAlive, Collide, Jump, Flag, Fireball, Desync
    )
    ...
    val fastestMode by choices("Fastest-SpeedChangeMode", arrayOf("Random", "Switch"), "Switch") { mode == "Fastest" }

    val fastestMotionA by float("Fastest-MotionA", 32f, 0f..320f) { mode == "Fastest" }
    val fastestMotionB by float("Fastest-MotionB", 16f, 0f..312f) { mode == "Fastest" }
    val fastestMotionY by float("Fastest-MotionY", 1f, 0f..64f) { mode == "Fastest" }
    ...
    val matrixMotionY1 by float("MatrixMotionY1", (-0.155f), -0.3f..0f) { mode == "MatrixTest" }
    val matrixMotionY2 by float("MatrixMotionY2", (-0.1f), -0.3f..0f) { mode == "MatrixTest" }
    val matrixTimerSpeed by float("MatrixTimer", 0.5f, 0.1f..1f) { mode == "MatrixTest" }
    val matrixMoveTick by int("MatrixMoveTick", 10, 0..30) { mode == "MatrixTest" }
    ...
    var boostMotion by float("BoostMotion", 6.5f, 1f..9.85f) { mode == "Verus" }
    ...
    // MatrixLag
    val zeroRot = boolean("ZeroRotation", false) {mode in arrayOf("MatrixLag")}
    val minFlags = int("MinFlags", 1, 1..5) {mode in arrayOf("MatrixLag")}
    var canJump = false
    var flag = false
    var flags = 0
    ...
    val horizontalSpeed by float("Tatako-HorizontalSpeed", 1f, 0f..5f) {mode in arrayOf("tatakoFly")}
    val verticalSpeed by float("Tatako-VerticalSpeed", 1f, 0f..5f) {mode in arrayOf("tatakoFly")}
    var x = 0.0
    var y: Double = 0.0
    var z: Double = 0.0
