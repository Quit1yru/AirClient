...
object TargetCommand : Command("target") {
    ...
    override fun execute(args: Array<String>) {
        ...
        when (args[1].lowercase()) {
            ...
            "villager" -> {
                ...
                chat("§7Target Villager toggled ${if (Targets.villager) "on" else "off"}.")
                playEdit()
            }
            "bedwarsmob" -> {
                Targets.bedwarsMob = !Targets.bedwarsMob
                chat("§7Target BedWarsMob toggled ${if (Targets.bedwarsMob) "on" else "off"}.")
                playEdit()
            }
            "invisible" -> {
    ...
    override fun tabComplete(args: Array<String>): List<String> {
        ...
        return when (args.size) {
            1 -> listOf("player", "mob", "animal", "invisible","villager","dead","bedwarsmob")
