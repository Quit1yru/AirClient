...
object Animations : Module("Animations", Category.RENDER, gameDetecting = false) {
    ...
    private val animations = arrayOf(
        ...
        ExhibitionAnimation(),
        SwingAnimation(),
        SimpleLargeAnimation(),
        ModernAnimation(),
        StableSpinAnimation(),
        LowkeyAssassinAnimation(),
        SimplePushdownAnimation()
    )
...
/**
 * 简洁放大动画 - 保持原有动画风格，只是简单放大
 * @author AI Assistant
 */
class SimpleLargeAnimation : Animation("SimpleLarge") {
    override fun transform(f1: Float, f: Float, clientPlayer: AbstractClientPlayer) {
        transformFirstPersonItem(f, f1)
        scale(0.6f, 0.6f, 0.6f) // 放大到0.6
        doBlockTransformations()
 ...
 * @author AI Assistant
 */
class ModernAnimation : Animation("Modern") {
    override fun transform(f1: Float, f: Float, clientPlayer: AbstractClientPlayer) {
        translate(0.56f, -0.52f, -0.72f)
        translate(0f, f * -0.6f, 0f)
        rotate(40f, 0f, 1f, 0f)

        val swing = MathHelper.sin(MathHelper.sqrt_float(f1) * 3.1415927f)
        rotate(swing * -25f, 0f, 0f, 1f)
        rotate(swing * -70f, 1f, 0f, 0f)
 ...
 * @author AI Assistant
 */
class StableSpinAnimation : Animation("StableSpin") {
 ...
 * @author AI Assistant
 */
class LowkeyAssassinAnimation : Animation("LowkeyAssassin") {
    override fun transform(f1: Float, f: Float, clientPlayer: AbstractClientPlayer) {
        translate(0.5f, -0.5f, -0.7f)

        val quickSwing = MathHelper.sin(f1 * 3.1415927f * 1.5f)
        rotate(quickSwing * -35f, 0f, 1f, 0f)
        rotate(quickSwing * -20f, 0f, 0f, 1f)
...
/**
 * 简洁下压动画 - 类似pushdown但更简洁
 * @author AI Assistant
 */
class SimplePushdownAnimation : Animation("SimplePushdown") {
    override fun transform(f1: Float, f: Float, clientPlayer: AbstractClientPlayer) {
        val swing = MathHelper.sin(MathHelper.sqrt_float(f1) * 3.1415927f)

        translate(0.56f, -0.52f + swing * 0.1f, -0.72f)
        translate(0f, f * -0.5f, 0f)
        rotate(45f, 0f, 1f, 0f)

        rotate(-swing * 50f, 1f, swing * 0.2f, 0f)
