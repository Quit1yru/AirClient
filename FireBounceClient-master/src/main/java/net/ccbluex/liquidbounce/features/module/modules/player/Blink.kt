...
object Blink : Module("Blink", Category.PLAYER, gameDetecting = false) {
    ...
    val onPacket = handler<PacketEvent> { event ->
        ...
        when (mode.lowercase()) {
            ...
            "received" -> {
                if (!allowS14) BlinkUtils.blink(packet, event, sent = false, receive = true) else BlinkUtils.blink(packet,event, sent = false, receive = true, receiveFilter = {p -> (p is S14PacketEntity)})
            }

            "both" -> {
                if (!allowS14) BlinkUtils.blink(packet, event) else BlinkUtils.blink(packet,event,receiveFilter = {p -> (p is S14PacketEntity)})
            }
        }
    }
