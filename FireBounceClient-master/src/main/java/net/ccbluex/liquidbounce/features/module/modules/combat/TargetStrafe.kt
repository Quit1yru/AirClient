...
object TargetStrafe : Module("TargetStrafe", Category.COMBAT) {
    private val thirdPersonViewValue = boolean("ThirdPersonView", false)
    private val renderModeValue = choices("RenderMode", arrayOf("Circle", "Polygon", "None"), "Polygon")
    ...
    private fun canStrafe(): Boolean {
        ...
        return state && targetEntity != null && (!holdSpaceValue.get() || mc.gameSettings.keyBindJump.isKeyDown) && (!onlySpeedValue.get() || FireBounce.moduleManager[Speed::class.java]?.state == true) && (!onlyFlightValue.get() || Fly.state)
    }
