...
object NoFall : Module("NoFall", Category.PLAYER) {
    ...
    val safe by boolean("Safe", false) {mode == "Matrix6.6.3-2"}
    ...
    var timered = false
    var fallDamageEnabled by boolean("FallDamage", true) { mode == "MatrixSpoof" }
    var inVoidCheck by boolean("VoidCheck", true) { mode == "MatrixSpoof" }
    var legitTimer by boolean("LegitTimer", true) { mode == "MatrixSpoof" }

    val simulateNoGround by boolean("SimulateNoGround",true) {mode == "NoGround"}
    ...
    val onMove = handler<MoveEvent> {
        ...
        if (collideBlock(thePlayer.entityBoundingBox) { it -> it is BlockLiquid }
            || collideBlock(
                fromBounds(
                    thePlayer.entityBoundingBox.maxX,
                    thePlayer.entityBoundingBox.maxY,
