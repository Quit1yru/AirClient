...
object OpaiIsland : Module("OpaiIsland", Category.RENDER) {
    ...
    private val styles by choices("Styles", arrayOf("None", "Normal", "Opal", "New Opai"), "Normal")
    ...
    val onRender2D = handler<Render2DEvent> {
        ...
        // 优先级判断：Chest > Scaffold > Notification > Normal
        ...
        } else if (scaffoldModule?.state == true && isScaffold) {
            renderMode = "SCAFFOLD"
            targetWidth = 190F
            targetHeight = 58F
            targetX = (width - targetWidth) / 2
        ...
        if (renderMode != "NONE") {
            ...
            val currentRadius = if (AnimGlobalHeight > 30F) {
                if (renderMode == "CHEST") ChestRounded else 8F
    ...
    private fun renderChestContent(x: Float, y: Float, w: Float, h: Float, slots: List<Slot>) {
        val padding = 8F
        val slotSize = 16
        val rippleDuration = 600L // 动画持续时间 (ms)
        val maxRadius = 16F // 最大扩散半径
        ...
        try {
            ...
            slots.forEachIndexed { index, slot ->
                ...
                // 仅当缓存中有旧数据时才比较（防止打开箱子瞬间全屏闪烁）
                if (prevSlotItems.containsKey(index)) {
                    val isChanged = if (stack == null && prevStack == null) {
                        false
                    } else if (stack == null || prevStack == null) {
                        true // 一个有一个无，变了
                    } else {
                        // 比较物品类型、NBT 以及 堆叠数量
                        !ItemStack.areItemStacksEqual(stack, prevStack) || stack.stackSize != prevStack.stackSize
                    }
                ...
                // 更新缓存 (必须 copy，否则引用的是同一个对象，属性变了无法检测)
            ...
            GlStateManager.disableDepth() // 确保在最上层
