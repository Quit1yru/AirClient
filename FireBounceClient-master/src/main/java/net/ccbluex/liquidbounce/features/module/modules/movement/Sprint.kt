...
object Sprint : Module("Sprint", Category.MOVEMENT, gameDetecting = false) {
    val tagMode by choices("TagMode",arrayOf("Normal","Custom","None"),"Normal")
    private val customText by text("CustomText","") {tagMode == "Custom"}
    val mode by choices("Mode", arrayOf("Legit", "Vanilla","Matrix","Intave"), "Vanilla")
    ...
    val jumpDirections by boolean("JumpDirections", false) { (mode == "Vanilla" && allDirections) || mode == "Matrix" }
    ...
    fun correctSprintState(movementInput: MovementInput, isUsingItem: Boolean) {
        ...
        if (MoreKB.breakSprint()) {
            player setSprintSafely false
            return
        }

        if (!handleEvents() || onlyOnSprintPress && !player.isSprinting && !mc.gameSettings.keyBindSprint.isKeyDown && !MoreKB.startSprint() && !isSprinting)
            return
        ...
        if (Scaffold2.handleEvents()) {
            if (!Scaffold2.sprint) {
                player setSprintSafely false
                isSprinting = false
                return
            } else if (Scaffold2.sprint && Scaffold2.eagle == "Normal" && player.isMoving && player.onGround && Scaffold2.eagleSneaking && Scaffold2.eagleSprint) {
                player setSprintSafely true
                isSprinting = true
                return
            }
        ...
        if (mc.thePlayer.isMoving && mode == "Matrix") {
            changeSprint(setState = true, sendPacketToServer = false)
        }
        if (mode == "Matrix" && mc.thePlayer.serverSprintState) {
    ...
    private fun shouldStopSprinting(movementInput: MovementInput, isUsingItem: Boolean): Boolean {
        ...
        val isIntaveModeActive = mode == "Intave"
        ...
        if (isIntaveModeActive) {

            return (Scaffold.state || Scaffold2.state) && modifiedForward < 0.8 ||
                    serverOpenInventory ||
                    (!NoSlow.handleEvents() && isUsingItem) ||
                    modifiedForward <= 0 ||
    ...
    val onPacket = handler<PacketEvent> { event ->
        ...
        if (packet is C0BPacketEntityAction && mode == "Matrix" && packet.action == C0BPacketEntityAction.Action.START_SPRINTING) {
            event.cancelEvent()
        }
