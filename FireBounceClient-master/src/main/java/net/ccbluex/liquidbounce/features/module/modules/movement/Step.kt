...
object Step : Module("Step", Category.MOVEMENT, gameDetecting = false) {
    ...
    private val mode by choices(
        ...
        arrayOf(
            ...
            "Spartan", "Rewinside", "BlocksMCTimer", "Matrix"
        ),
        "NCP"
    )

    private val height by float("Height", 1F, 0.6F..10F)
    { mode !in arrayOf("Jump", "MotionNCP", "LAAC", "AAC3.3.4", "BlocksMCTimer", "Matrix") }
    ...
    private val twoBlock by boolean("2Block", true) { mode == "Matrix" }
    private val instant by boolean("Instant", true) { mode == "Matrix" && twoBlock }
    ...
    // Matrix specific values
    private var matrixTicks = 0
    private var matrixDoJump = false
    ...
    val onUpdate = loopSequence {
        ...
        // Matrix step handling
        if (mode == "Matrix") {
            thePlayer.stepHeight = if (twoBlock) 2.0F else 1.0F

            if (matrixDoJump) {
                if ((matrixTicks > 0 && thePlayer.onGround) || matrixTicks > 5) {
                    matrixTicks = 0
                    matrixDoJump = false
                    return@loopSequence
                }
    ...
    val onStep = handler<StepEvent> { event ->
        ...
        // Matrix step handling
        if (mode == "Matrix") {
            if (event.stepHeight > 1.0F) {
                if (instant) {
                    // Send 2 block step packets
                    sendPackets(
                        C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 0.41999998688698, thePlayer.posZ, false),
                        C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 0.7531999805212, thePlayer.posZ, false),
                        C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 1.00133597911215, thePlayer.posZ, true),
                        C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 1.42133596599913, thePlayer.posZ, false),
                        C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 1.75453595963335, thePlayer.posZ, false),
                        C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 2.0026719582243, thePlayer.posZ, false)
                    )
            ...
            if (event.stepHeight > 0.6F) {
                ...
                sendPackets(
                    C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 0.42F, thePlayer.posZ, false),
                    C04PacketPlayerPosition(thePlayer.posX, thePlayer.posY + 0.42F, thePlayer.posZ, true)
                )
            }
            return@handler
        }
