...
object AutoProjectile : Module("AutoProjectile", Category.COMBAT) {
    ...
    private val range by floatRange("Range", 0F..8F, 1F..20F)
    private val throwDelay by intRange("ThrowDelay", 1000..1500, 50..2000)

    private val switchBackDelay by intRange("SwitchBackDelay", 500..500, 50..2000)
    ...
    val onUpdate = handler<UpdateEvent> {
        ...
        if (usingProjectile) {
            if (projectilePullTimer.hasTimePassed(switchBackDelay.random())) {
            ...
            if (facingEnemy) {
                ...
                if (facingEntity == null) {
                    facingEntity = raycastEntity(range.endInclusive.toDouble()) { isSelected(it, true) }
                }
                if (facingEntity != null) {
                    val distance = facingEntity.let { entity -> mc.thePlayer.getDistanceToEntityBox(facingEntity) }
                    if (distance <= range.start) {
                        facingEntity = null
                    }
                }
