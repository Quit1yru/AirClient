...
object ProjectileAimbot : Module("ProjectileAimbot", Category.COMBAT) {
    ...
    private val fishingRod by boolean("FishingRod", true).subjective() // Added fishing rod option
    ...
    val onRotationUpdate = handler<RotationUpdateEvent> {
        ...
        val targetRotation = when (val item = player.heldItem?.item) {
            ...
            is ItemFishingRod -> { // Added fishing rod case
                if (!fishingRod || !player.isUsingItem)
                    return@handler

                target = getTarget(throughWalls, priority)

                // Fishing rods have different projectile properties
                faceTrajectory(target ?: return@handler, predict, predictSize, gravity = 0.04f, velocity = 1.5f)
            }

            is Item -> {
                if (!otherItems && !player.heldItem.isEmpty() ||
                    (!egg && item is ItemEgg ||
                            !snowball && item is ItemSnowball ||
                            !pearl && item is ItemEnderPearl ||
                            !fishingRod && item is ItemFishingRod) // Added fishing rod check
                )
                    return@handler
