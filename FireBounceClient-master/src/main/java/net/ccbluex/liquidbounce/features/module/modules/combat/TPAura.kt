...
object TPAura : Module("TPAura", Category.COMBAT) {
    ...
    //private val positionPredict by boolean("PositionPredict", true)
    //private val predictTick by int("PredictTick", 1, 1..5){positionPredict&&!predictTickBasedOnPlayerPing}
    //private val predictTickBasedOnPlayerPing by boolean("PredictTickBasedOnPlayerPing", true){positionPredict}
    ...
    private val noMovePacket by boolean("NoMovePacket", false) //一次攻击周期太长中间可能发出一些我们不希望的发包(由MineCraft产生)
    ...
    val onPacket = handler<PacketEvent> { event ->
        ...
        if((packet is C03PacketPlayer)&&noMovePacket){
            ...
            } //it means tp aura is working, so it will sync position automatically
        }
    ...
    private fun runAttack() {
        ...
        while(targets.isNotEmpty()){
            ...
            //val pt = if(predictTickBasedOnPlayerPing)  (mc.thePlayer.getPing()/50)+1 else predictTick //你发送的数据包到达的时候对方已经又走了这么多tick
            val pt = (mc.thePlayer.getPing()/50)+1 //服务器晚1tick处理玩家数据包
