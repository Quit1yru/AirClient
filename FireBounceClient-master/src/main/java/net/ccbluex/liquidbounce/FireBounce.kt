...
import net.ccbluex.liquidbounce.features.special.ClientRichPresence.mc
...
import net.ccbluex.liquidbounce.file.configs.models.ClientConfiguration.showPlayTime
import net.ccbluex.liquidbounce.file.configs.models.ClientConfiguration.showRealPlayer
...
object FireBounce {
     ...
     */
    const val CLIENT_NAME = "FireBounce"
    const val CLIENT_AUTHOR = "CCBlueX/FireFly_Legit/TianYang/0x16z/H1d3n"
    const val CLIENT_CLOUD = "https://cloud.liquidbounce.net/LiquidBounce"
    const val CLIENT_WEBSITE = "liquidbounce.net"
    const val CLIENT_GITHUB = "https://github.com/FireFlyUnlegit/FireBounceClient"
    ...
    @JvmField
    var clientTitle = "FireBounceClient $clientVersionText | Development Version"
     ...
     * 获取已游玩的秒数
     */
    fun getPlayTimeSeconds(): Int {
        return if (isPlaying && startTimeMillis > 0) {
    ...
    fun getDynamicTitle(): String {
        val actualPlayerName = if (showRealPlayer) mc.session.username?: "None" else ""
        val actualTimeText = if (showPlayTime) String.format("%02d:%02d:%02d", 
            (getPlayTimeSeconds() / 3600), 
            (getPlayTimeSeconds() % 3600) / 60, 
            getPlayTimeSeconds() % 60) else ""
        return if (isPlaying) {
            "$clientTitle | Play Time: $actualTimeText | Player: $actualPlayerName"
        } else {
            clientTitle
        }
    }
