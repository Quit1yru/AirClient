...
import net.ccbluex.liquidbounce.features.module.modules.render.KeepBlockingRender;
...
public abstract class MixinEntityPlayer extends MixinEntityLivingBase {
    ...
    private double injectKeepSprintA(double constant) {
        if (NewVelocity.canCancelHitSlow) {
            NewVelocity.canCancelHitSlow = false;
            return 1.0;
        }
        if (KeepSprint.INSTANCE.getShouldKeepSprint()) {
            return KeepSprint.INSTANCE.getMotionAfterAttack();
        }
    ...
    private void injectGetItemInUseCount(CallbackInfoReturnable<Integer> cir) {
        ...
        final KeepBlockingRender keepBlockingRender = KeepBlockingRender.INSTANCE;

        if ((Object) this == mc.thePlayer) {
            ItemStack stack = mc.thePlayer.getHeldItem();
            if (keepBlockingRender != null && KeepBlockingRender.shouldShowRender && keepBlockingRender.handleEvents()) {
