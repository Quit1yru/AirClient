...
import net.ccbluex.liquidbounce.event.HurtEvent;
import net.ccbluex.liquidbounce.event.JumpEvent;
import net.ccbluex.liquidbounce.features.module.modules.combat.Fixes;
...
public abstract class MixinEntityLivingBase extends MixinEntity {
    ...
    protected void jump() {
        ...
        if (isSprinting() && (Minecraft.getMinecraft().thePlayer.moveForward != 0 || Minecraft.getMinecraft().thePlayer.moveStrafing != 0)) {
            ...
            } else if (sprint.getMode().equals("Matrix") && sprint.getJumpDirections()) {
                fixedYaw += MathExtensionsKt.toDegreesF(MovementUtils.INSTANCE.getDirection()) - this.rotationYaw;
            }
    ...
    private void headLiving(CallbackInfo callbackInfo) {
        if (
                NoJumpDelay.INSTANCE.handleEvents() ||
                        (Fixes.noJumpDelay && Fixes.INSTANCE.handleEvents()) ||
                        Scaffold.INSTANCE.handleEvents() &&
                                Tower.INSTANCE.getTowerModeValues().toString().equals("Pulldown")
        ) jumpTicks = 0;
    }
    ...
    @Inject(method = "attackEntityFrom", at = @At(value = "FIELD",
            target = "Lnet/minecraft/entity/EntityLivingBase;hurtTime:I", ordinal = 0, shift = At.Shift.AFTER, opcode = Opcodes.PUTFIELD))
    private void onHurtTimeSet(DamageSource source, float amount, CallbackInfoReturnable<Boolean> cir) {
        EntityLivingBase entity = (EntityLivingBase) (Object) this;
        EventManager.INSTANCE.call(new HurtEvent(entity, this.hurtTime));
    }
