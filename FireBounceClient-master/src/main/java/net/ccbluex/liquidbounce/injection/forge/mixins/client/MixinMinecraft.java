...
public abstract class MixinMinecraft {
    ...
    private void init(CallbackInfo callbackInfo) {
        ...
        liquidBounce$preloadFuture = FireBounce.INSTANCE.preload();
    }
    ...
    private void startGame(CallbackInfo callbackInfo) throws ExecutionException, InterruptedException {
        ...
        FireBounce.INSTANCE.startClient();
    }
    ...
    private void createDisplay(CallbackInfo callbackInfo) {
        if (ClientConfiguration.INSTANCE.getClientTitle()) {
            Display.setTitle(FireBounce.clientTitle);
        }
    }
    ...
    @Unique
    private long liquidBounce$lastSecondTick = System.currentTimeMillis();

    @Inject(method = "runGameLoop", at = @At("HEAD"))
    private void injectSecondTick(CallbackInfo ci) {
        long currentTime = System.currentTimeMillis();
        // 每秒触发一次
        if (currentTime - liquidBounce$lastSecondTick >= 1000) {
            EventManager.INSTANCE.call(SecondTickEvent.INSTANCE);
            liquidBounce$lastSecondTick = currentTime;
        }
    }
    ...
    private void injectSkipTicks(CallbackInfo ci) {
        // 检查两种跳过方式：直接跳过和通过 TickSkipManager
        if (skipTicks > 0) {
            skipTicks--;
            ci.cancel();
            return;
        ...
        // 新增：检查 TickSkipManager 的跳过
        if (TickSkipManager.INSTANCE.shouldSkipTick()) {
            ci.cancel();
        }
    }
    ...
    private void shutdown(CallbackInfo callbackInfo) {
        FireBounce.INSTANCE.stopClient();
    }
    ...
    private void loadWorld(WorldClient p_loadWorld_1_, String p_loadWorld_2_, final CallbackInfo callbackInfo) {
        ...
        EventManager.INSTANCE.call(new WorldEvent(p_loadWorld_1_, p_loadWorld_2_ ));
    }
