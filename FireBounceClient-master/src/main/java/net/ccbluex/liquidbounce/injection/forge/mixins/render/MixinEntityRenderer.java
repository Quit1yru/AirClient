...
import net.ccbluex.liquidbounce.features.module.modules.world.scaffolds.Scaffold2; // 导入 Scaffold2
...
public abstract class MixinEntityRenderer {
    ...
    /**
     * 隐藏潜行摄像机偏移 - 支持 Scaffold 和 Scaffold2
     */
    @Redirect(method = "orientCamera", at = @At(value = "INVOKE", target = "Lnet/minecraft/entity/Entity;getEyeHeight()F"))
    private float modifyEyeHeightForSneak(Entity entity) {
        float originalHeight = entity.getEyeHeight();
        if (entity != mc.thePlayer) {
            return originalHeight;
        }

        Scaffold scaffold = Scaffold.INSTANCE;
        Scaffold2 scaffold2 = Scaffold2.INSTANCE;

        boolean shouldHideSneak = scaffold.getState() && shouldHideSneakOffset(scaffold);

        if (scaffold2.getState() && shouldHideSneakOffset(scaffold2)) {
            shouldHideSneak = true;
        }

        if (shouldHideSneak && entity.isSneaking()) {
     ...
     */
    @Unique
    private boolean shouldHideSneakOffset(Object scaffoldModule) {
        try {
            // 使用反射获取 hideSneak 字段值
            Field hideSneakField = scaffoldModule.getClass().getDeclaredField("hideSneak");
            hideSneakField.setAccessible(true);
            boolean hideSneak = (boolean) hideSneakField.get(scaffoldModule);

            // 获取 eagle 字段值
            Field eagleField = scaffoldModule.getClass().getDeclaredField("eagle");
            eagleField.setAccessible(true);
            Object eagleValue = eagleField.get(scaffoldModule);
            String eagle = eagleValue.toString();

            // 获取 scaffoldMode 字段值
            Field scaffoldModeField = scaffoldModule.getClass().getDeclaredField("scaffoldMode");
            scaffoldModeField.setAccessible(true);
            Object scaffoldModeValue = scaffoldModeField.get(scaffoldModule);
            String scaffoldMode = scaffoldModeValue.toString();

            // 检查条件：hideSneak 为 true，eagle 为 "Normal"，scaffoldMode 不是 "GodBridge"
            return hideSneak && "Normal".equals(eagle) && !"GodBridge".equals(scaffoldMode);
