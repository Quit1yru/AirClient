...
class IntValue(
    ...
    suffix: String? = null,
    description: String? = null
) : Value<Int>(name, value, suffix, description = description) {
...
class IntRangeValue(
    ...
    suffix: String? = null,
    description: String? = null
) : Value<IntRange>(name, value, suffix, description = description) {
...
class FloatValue(
    ...
    suffix: String? = null,
    description: String? = null
) : Value<Float>(name, value, suffix, description = description) {
...
class FloatRangeValue(
    ...
    suffix: String? = null,
    description: String? = null
) : Value<ClosedFloatingPointRange<Float>>(name, value, suffix, description = description) {
...
class FontValue(
    ...
    value: FontRenderer,
    description: String? = null
) : Value<FontRenderer>(name, value, description = description) {
...
class BlockValue(
    ...
    value: Int,
    description: String? = null,
    val range: IntRange = 1..197,
) : Value<Int>(name, value, suffix = null, description = description) {
...
class MultiChoiceValue(
    ...
    val forceChosenChoices: List<String> = emptyList()
) : Value<List<String>>(name, value.ifEmpty { listOf(choices.first()) }, description = description)
{
    ...
    override fun toJson(): JsonElement {
        return JsonArray().apply {
            get().forEach { add(JsonPrimitive(it)) }
        }
    }

    override fun fromJsonF(element: JsonElement): List<String>? {
        val list = (element as? JsonArray)?.map { it.asString }?.toList() ?: return null
        return ensureForceChosenIncluded(list)
    }
    ...
    override fun fromTextF(text: String): List<String>? {
        val list = text.split(",").map { it.trim() }.filter { it.isNotEmpty() }
        return ensureForceChosenIncluded(list)
    }

    override fun validate(newValue: List<String>): List<String> {
        val validated = newValue.filter { it in choices }.distinct()
        return ensureForceChosenIncluded(validated)
    }

    fun set(value: List<String>) {
        val validatedValue = validate(value)
        super.set(validatedValue, true)
    }

    fun toggle(choice: String) {
        val current = get().toMutableList()

        if (choice in forceChosenChoices && current.contains(choice)) {
            return
        }

        if (current.contains(choice)) {
            if (current.size > 1 || current.any { it in forceChosenChoices }) {
    ...
    fun isSelected(choice: String): Boolean {
        return get().contains(choice)
    }

    fun isForceChosen(choice: String): Boolean {
        return choice in forceChosenChoices
    }

    fun getSelectableChoices(): List<String> {

        return choices.filter { it !in forceChosenChoices }
    }

    operator fun contains(string: String?) = choices.any { it.equals(string, true) }

    private fun ensureForceChosenIncluded(list: List<String>): List<String> {
        val result = list.toMutableList()
        forceChosenChoices.forEach { forceChoice ->
            if (forceChoice !in result && forceChoice in choices) {
                result.add(forceChoice)
            }
        }
        return if (result.isEmpty()) {
            if (forceChosenChoices.isNotEmpty()) forceChosenChoices.take(1) else listOf(choices.first())
        } else {
            result.distinct()
        }
    }
    ...
    init {
        val initialValue = ensureForceChosenIncluded(value)
        if (initialValue != value) {
            super.set(initialValue, true)
        }
    }
}
...
class ColorValue(
    ...
    description: String? = null
) : Value<Color>(name, defaultColor, description = description) {
    // Sliders
    var hueSliderY = 0F
