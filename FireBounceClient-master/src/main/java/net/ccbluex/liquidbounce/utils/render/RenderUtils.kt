...
import net.ccbluex.liquidbounce.utils.extra.ColorUtils.clearColor
import net.ccbluex.liquidbounce.utils.extra.ColorUtils.setColor
...
object RenderUtils : MinecraftInstance {
    ...
    fun renderItem(itemStack: ItemStack, x: Int, y: Int, withOverlay: Boolean = true, alpha: Int = 255) {
        glPushMatrix()
        glEnable(GL_BLEND)
        ...
        if (withOverlay) {
            renderItem.renderItemOverlays(mc.fontRendererObj, itemStack, x, y)
        }
        ...
        if (alpha < 255) {
            drawRect(x, y, x + 16, y + 16, Color(0, 0, 0, 255 - alpha).rgb)
        }

        glDisable(GL_BLEND)
        glPopMatrix()
    }
    fun drawBloom(x: Float, y: Float, width: Float, height: Float, radius: Float, color: Color) {
        drawRoundedRect(x, y, x + width, y + height, color.rgb, radius)
    }

    fun drawBloom(x: Int, y: Int, width: Int, height: Int, radius: Int, color: Color) {
        drawBloom(x.toFloat(), y.toFloat(), width.toFloat(), height.toFloat(), radius.toFloat(), color)
    }

    fun drawRoundedOutline(x: Float, y: Float, x2: Float, y2: Float, radius: Float, width: Float, color: Int) {
        drawRoundedBorder(x, y, x2, y2, width, color, radius)
    }
    ...
    fun drawCustomShapeWithRadiusRetangle(
        ...
        glPopAttrib()
        net.ccbluex.liquidbounce.utils.extra.ColorUtils.clearColor()
    }
    ...
    fun drawRoundedGradientRectCorner(
        ...
        val (newX1, newY1, newX2, newY2) = orderPoints(left, top, right, bottom)
        val radiusD = min(radius.toDouble(), min(newX2 - newX1, newY2 - newY1) / 2.0)

        // 分解颜色
        val startAlpha = (startColor shr 24 and 255) / 255.0f
        val startRed = (startColor shr 16 and 255) / 255.0f
        val startGreen = (startColor shr 8 and 255) / 255.0f
        val startBlue = (startColor and 255) / 255.0f

        val endAlpha = (endColor shr 24 and 255) / 255.0f
        val endRed = (endColor shr 16 and 255) / 255.0f
        val endGreen = (endColor shr 8 and 255) / 255.0f
        val endBlue = (endColor and 255) / 255.0f

        drawWithTessellatorWorldRenderer {
            begin(GL_TRIANGLE_FAN, DefaultVertexFormats.POSITION_COLOR)

            // 中心点 - 使用起始颜色
            pos((newX1 + newX2) / 2.0, (newY1 + newY2) / 2.0, 0.0).color(startRed, startGreen, startBlue, startAlpha).endVertex()
            ...
            for (i in 0..360 step 10) {
                ...
                // 计算当前角度在矩形边界上的位置
                val edgeX = when {
                    cos > 0.5 -> newX2 - radiusD
                    cos < -0.5 -> newX1 + radiusD
                    else -> newX1 + radiusD + (newX2 - newX1 - 2 * radiusD) * (cos + 0.5)
                }

                val edgeY = when {
                    sin > 0.5 -> newY2 - radiusD
                    sin < -0.5 -> newY1 + radiusD
                    else -> newY1 + radiusD + (newY2 - newY1 - 2 * radiusD) * (sin + 0.5)
                }

                // 计算圆角部分的点
                val cornerX = when {
                    cos > 0.5 -> newX2 - radiusD + radiusD * (cos - 0.5) * 2
                    cos < -0.5 -> newX1 + radiusD + radiusD * (cos + 0.5) * 2
                    else -> edgeX
                }

                val cornerY = when {
                    sin > 0.5 -> newY2 - radiusD + radiusD * (sin - 0.5) * 2
                    sin < -0.5 -> newY1 + radiusD + radiusD * (sin + 0.5) * 2
                    else -> edgeY
                }

                // 根据位置插值颜色（简单的水平渐变）
                val colorProgress = (cornerX - newX1) / (newX2 - newX1)
                val currentRed = startRed + (endRed - startRed) * colorProgress.toFloat()
                val currentGreen = startGreen + (endGreen - startGreen) * colorProgress.toFloat()
                val currentBlue = startBlue + (endBlue - startBlue) * colorProgress.toFloat()
                val currentAlpha = startAlpha + (endAlpha - startAlpha) * colorProgress.toFloat()

                pos(cornerX, cornerY, 0.0).color(currentRed, currentGreen, currentBlue, currentAlpha).endVertex()
            }

            // 闭合图形
            val firstAngle = Math.toRadians(0.0)
            val firstCos = cos(firstAngle)
            val firstSin = sin(firstAngle)

            val firstEdgeX = when {
                firstCos > 0.5 -> newX2 - radiusD
                firstCos < -0.5 -> newX1 + radiusD
                else -> newX1 + radiusD + (newX2 - newX1 - 2 * radiusD) * (firstCos + 0.5)
            }

            val firstEdgeY = when {
                firstSin > 0.5 -> newY2 - radiusD
                firstSin < -0.5 -> newY1 + radiusD
                else -> newY1 + radiusD + (newY2 - newY1 - 2 * radiusD) * (firstSin + 0.5)
            }

            val firstCornerX = when {
                firstCos > 0.5 -> newX2 - radiusD + radiusD * (firstCos - 0.5) * 2
                firstCos < -0.5 -> newX1 + radiusD + radiusD * (firstCos + 0.5) * 2
                else -> firstEdgeX
            }

            val firstCornerY = when {
                firstSin > 0.5 -> newY2 - radiusD + radiusD * (firstSin - 0.5) * 2
                firstSin < -0.5 -> newY1 + radiusD + radiusD * (firstSin + 0.5) * 2
                else -> firstEdgeY
            }

            val firstColorProgress = (firstCornerX - newX1) / (newX2 - newX1)
            val firstCurrentRed = startRed + (endRed - startRed) * firstColorProgress.toFloat()
            val firstCurrentGreen = startGreen + (endGreen - startGreen) * firstColorProgress.toFloat()
            val firstCurrentBlue = startBlue + (endBlue - startBlue) * firstColorProgress.toFloat()
            val firstCurrentAlpha = startAlpha + (endAlpha - startAlpha) * firstColorProgress.toFloat()

            pos(firstCornerX, firstCornerY, 0.0).color(firstCurrentRed, firstCurrentGreen, firstCurrentBlue, firstCurrentAlpha).endVertex()
        }
    ...
    fun drawBoundingBox(minX: Double, minY: Double, minZ: Double, maxX: Double, maxY: Double, maxZ: Double) {
        drawWithTessellatorWorldRenderer {
    ...
    fun renderNameTag(string: String, x: Double, y: Double, z: Double) {
        ...
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        val width = Fonts.Bold36.getStringWidth(string) / 2
        drawRect(-width - 1, -1, width + 1, Fonts.Bold36.FONT_HEIGHT, Int.MIN_VALUE)
        Fonts.Bold36.drawString(string, -width.toFloat(), 1.5f, Color.WHITE.rgb, true)
        resetCaps()
        resetColor()
        glPopMatrix()
    }
    ...
    @JvmStatic
    fun setup2DRenderingGLUtil(runnable: Runnable) {
        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glDisable(GL_TEXTURE_2D)
        runnable.run()
    ...
    @JvmStatic
    fun renderGLUtil(mode: Int, runnable: Runnable) {
        glBegin(mode)
        runnable.run()
        glEnd()
    }
