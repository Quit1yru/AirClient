...
object MiscUtils : MinecraftInstance {
    ...
    fun generateCrashInfo(): String {
        var base = """
            --- Game crash info ---
            Client: FireBounce ${FireBounce.clientVersionText} (${FireBounce.clientCommit})
    ...
    fun Throwable.showErrorPopup(
        ...
        val openIssueButton = JButton("Open GitHub Issue").apply {
            addActionListener {
                showURL("${FireBounce.CLIENT_GITHUB}/issues/new?template=bug_report.yml&title=%5BBUG%5D+Game+crashed+$exceptionType")
            }
        }
...
object FileFilters {
    ...
    @JvmField
    val LUA = FileNameExtensionFilter("Lua Script Files", "lua")
}
