...
// Extension for FDP dropdown compatibility
...
fun interpolateFloat(current: Float, target: Float, speed: Double): Float {
    return (current + (target - current) * speed).toFloat()
}

fun java.awt.Color.withAlpha(alpha: Int): java.awt.Color {
    return java.awt.Color(red, green, blue, alpha.coerceIn(0, 255))
}
...
fun roundX(value: Double, step: Double): Double {
    return round(value, step)
}

@kotlin.jvm.JvmName("roundToHalfD")
fun roundToHalf(d: Double) = d.roundToHalf()
@kotlin.jvm.JvmName("roundToHalfF")
fun roundToHalf(f: Float) = f.roundToHalf()