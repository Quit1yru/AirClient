...
object MovementUtils : MinecraftInstance {
    ...
    fun strafe(event: MoveEvent, speed: Float) {
        if (!mc.thePlayer.isMoving) return
       event.x = -sin(MovementUtils.direction) * speed
       event.z = cos(MovementUtils.direction) * speed
    }
    ...
    val movingYaw: Float
        get() = (MovementUtils.direction * 180f / Math.PI).toFloat()
