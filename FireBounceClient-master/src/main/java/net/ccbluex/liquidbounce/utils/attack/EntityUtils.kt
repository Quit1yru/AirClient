...
object EntityUtils : MinecraftInstance {
    ...
    object Targets : Configurable("Targets") {
        ...
        var villager by boolean("Village", false)
        var bedwarsMob by boolean("BedWarsMob",true)
        var invisible by boolean("Invisible", false)
        var dead by boolean("Dead", true)
    ...
    fun isSelected(entity: Entity?, canAttackCheck: Boolean): Boolean {
        if (entity is EntityLivingBase && (Targets.dead || entity.isEntityAlive) && entity != mc.thePlayer) {
            if (Targets.invisible || !entity.isInvisible) {
                ...
                return (Targets.mob && entity.isMob() && !entity.isVillager() && !entity.isBedWars()) ||
                        (Targets.animal && entity.isAnimal()) ||
                        (Targets.villager && entity.isVillager()) ||
                        (Targets.bedwarsMob && entity.isBedWars())
            }
        }
        return false
    }
