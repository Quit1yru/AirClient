...
object InventoryUtils : MinecraftInstance, Listenable {
    ...
    fun countItems(item: Item): Int {
        val player = mc.thePlayer ?: return 0
        var count = 0

        // Check inventory slots 0-35 (inventory + hotbar)
        for (i in 0..35) {
    ...
    fun countItems(itemId: Int): Int {
        val player = mc.thePlayer ?: return 0
        var count = 0

        for (i in 0..35) {
            val stack = player.inventory.getStackInSlot(i)
            if (stack != null && Item.getIdFromItem(stack.item) == itemId) {
                count += stack.stackSize
            }
        }
