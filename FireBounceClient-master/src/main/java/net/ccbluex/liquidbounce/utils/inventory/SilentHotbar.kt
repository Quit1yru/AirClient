...
import net.ccbluex.liquidbounce.utils.inventory.SilentHotbar.currentSlot
import net.ccbluex.liquidbounce.utils.inventory.SilentHotbar.hotbarState
...
object SilentHotbar : Listenable, MinecraftInstance {
    ...
    fun getSilentSlotItemType(): Item? {
        val currentSlot = SilentHotbar.currentSlot
        val player = mc.thePlayer ?: return null

        // 获取当前 Silent 槽位对应的物品
        val itemStack = player.inventory.getStackInSlot(currentSlot)
    ...
    fun getVisualSlotItemType(): Item? {
        val player = mc.thePlayer ?: return null
        val visualSlotIndex = renderSlot(true)
        return player.inventory.getStackInSlot(visualSlotIndex)?.item
    }
