...
import net.ccbluex.liquidbounce.features.module.modules.combat.NewVelocity
import net.ccbluex.liquidbounce.utils.attack.CPSCounter
import net.ccbluex.liquidbounce.utils.attack.SilentAttackManager
...
import net.ccbluex.liquidbounce.utils.extensions.setSprintWithSync
import net.ccbluex.liquidbounce.utils.inventory.attackDamage
import net.ccbluex.liquidbounce.utils.inventory.attackDamageWithCritical
import net.ccbluex.liquidbounce.utils.kotlin.RandomUtils
import net.ccbluex.liquidbounce.utils.math.ComplexMath
import net.ccbluex.liquidbounce.utils.math.MathFunction.pow
...
object SomeUtil {
    ...
    fun changeSprint(setState: Boolean, sendPacketToServer: Boolean = true, forceChange: Boolean = false) {
        if (forceChange) {
            when (sendPacketToServer) {
                true -> mc.thePlayer setSprintWithSync setState

                false -> mc.thePlayer setSprintSafely setState
            }
            return
        }
        mc.thePlayer.isSprinting = setState
        mc.thePlayer.serverSprintState = sendPacketToServer
    ...
    fun keepingSprint(): Boolean {
        if (KillAura.shouldKeepSprint) return true
        if (NewVelocity.canCancelHitSlow) return true
        if (mc.thePlayer.hurtTime !in KeepSprint.workHurtTime) return false
        if (KeepSprint.BurningCheck && mc.thePlayer.isBurning) return false
        if (KeepSprint.WaterCheck && mc.thePlayer.isInWater) return false
        if (KeepSprint.LavaCheck && mc.thePlayer.isInLava) return false
        if (KeepSprint.CobwebCheck && mc.thePlayer.isInWeb) return false
        if (KeepSprint.SlowDownPotionCheck && mc.thePlayer.isPotionActive(Potion.moveSlowdown)) return false
        if (KeepSprint.SpeedPotionCheck && mc.thePlayer.isPotionActive(Potion.moveSpeed)) return false
    ...
    val EntityPlayerSP.bps: Double
        get() = sqrt((mc.thePlayer.velocityX * mc.thePlayer.velocityX + mc.thePlayer.velocityZ * mc.thePlayer.velocityZ)) * 20.0

    val EntityPlayerSP.bpt: Double
        get() = ComplexMath.hypot(mc.thePlayer.velocityX, mc.thePlayer.velocityZ)
    val EntityPlayerSP.velocityX: Double
        get() = mc.thePlayer.motionX
    val EntityPlayerSP.velocityY: Double
        get() = mc.thePlayer.motionY
    val EntityPlayerSP.velocityZ: Double
        get() = mc.thePlayer.motionZ
    fun EntityPlayerSP.setBPSTo(targetBPS: Double) {
        if (mc.thePlayer.bps != 0.0) reduceXZ((targetBPS / bps))
    }
    ...
    fun getCurrentWeaponDamage(isCritical: Boolean): Double {
        ...
        var damage = if (!isCritical) 1.0 + heldItem.attackDamage else 1.5 + heldItem.attackDamageWithCritical
