...
fun isValidInput(typedChar: Char, text: EditableText): Boolean {
    ...
    return when (text.value) {
        ...
        is IntValue -> {
            val isDigit = typedChar.isDigit()
            val isMinus = typedChar == '-' && text.string.isEmpty() && text.value.minimum < 0
            (isDigit || isMinus) && text.validator(nextString)
        }
        is FloatValue -> {
            val isDigit = typedChar.isDigit()
            val isDot = typedChar == '.' && !text.string.contains('.')
            val isMinus = typedChar == '-' && text.string.isEmpty() && text.value.minimum < 0
            (isDigit || isDot || isMinus) && text.validator(nextString)
        }
...
data class EditableText(
    ...
    companion object {
        ...
        fun forIntValue(value: IntValue) = EditableText(
            value = value,
            string = value.get().toString(),
            validator = { text ->
                if (text.isEmpty()) true
                else text.toIntOrNull()?.let { it in value.range } ?: false
            },
            onUpdate = { newText ->
                newText.toIntOrNull()?.takeIf { it in value.range }?.let { value.set(it) }
            }
        )

        fun forFloatValue(value: FloatValue) = EditableText(
            value = value,
            string = value.get().toString(),
            validator = { text ->
                if (text.isEmpty()) true
                else text.toFloatOrNull()?.let { it in value.range } ?: false
            },
            onUpdate = { newText ->
                newText.toFloatOrNull()?.takeIf { it in value.range }?.let { value.set(it) }
            }
        )
