...
object RenderUtils : MinecraftInstance {
    ...
    fun drawHead(
        ...
        drawHead(skin, x, y, 8f, 8f, 8, 8, width, height, 64f, 64f, color)
    }
    ...
    fun drawGlassmorphism(
        x: Float, y: Float, width: Float, height: Float, radius: Float, blurStrength: Float,
        glassBaseColor: Color = Color(200, 220, 255, 15),
        glassHighlightAlpha: Int = 20,
        glassBorderColor: Color = Color(255, 255, 255, 40)
    ) {
        val x2 = x + width
        val y2 = y + height

        BlurUtils.blurAreaRounded(x, y, x2, y2, radius, blurStrength)

        drawRoundedRect(x, y, x2, y2, glassBaseColor.rgb, radius)

        drawRoundedRect(x, y, x2, y + height * 0.3f, Color(255, 255, 255, glassHighlightAlpha).rgb, radius)

        try {
            drawRoundedBorder(x, y, x2, y2, 1f, glassBorderColor.rgb, radius)
        } catch (e: Exception) {
        }
    }

    fun drawNeonBorder(
        x: Float, y: Float, width: Float, height: Float, radius: Float, neonColor: Color,
        blurStrength: Float = 5f,
        outerGlowAlpha: Int = 50,
        innerBorderAlpha: Int = 180,
        backgroundAlpha: Int = 80
    ) {
        val x2 = x + width
        val y2 = y + height
        ...
        drawRoundedRect(x, y, x2, y2, Color(0, 0, 0, backgroundAlpha).rgb, radius)

        try {
            drawRoundedBorder(x - 1, y - 1, x2 + 1, y2 + 1, 3f, Color(neonColor.red, neonColor.green, neonColor.blue, outerGlowAlpha).rgb, radius + 1)
        } catch (e: Exception) {
        }

        try {
            drawRoundedBorder(x, y, x2, y2, 1f, Color(neonColor.red, neonColor.green, neonColor.blue, innerBorderAlpha).rgb, radius)
        } catch (e: Exception) {
        }
    }
    ...
    fun customRotatedObject2D(x: Float, y: Float, width: Float, height: Float, rotation: Double) {
        ...
        GlStateManager.translate(centerX.toDouble(), centerY.toDouble(), 0.0)
        GlStateManager.rotate(rotation.toFloat(), 0f, 0f, 1f)
        GlStateManager.translate(-centerX.toDouble(), -centerY.toDouble(), 0.0)
    }
    ...
    fun setupDrawCircles(render: Runnable) {
        ...
        GlStateManager.alphaFunc(GL_GREATER, .1f)
        GlStateManager.enableAlpha()
